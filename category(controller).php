		...
		if (isset($this->request->get['path'])) {
            ...
            $category_draw = $this->model_catalog_category->getCategoryDrawing($category_id);
                $drawing = explode(",",$category_draw['0']['drawing']);
                    if ($drawing['0']) {
                    //$data['thumb_draw'] = $this->model_tool_image->resize($drawing['0'], $this->config->get($this->config->get('config_theme') . '_image_category_width'), $this->config->get($this->config->get('config_theme') . '_image_category_height'));
...
//End creat module "Tools for Category"	
			...
			$limits = array_unique(array($this->config->get($this->config->get('config_theme') . '_product_limit'), 100));
			sort($limits);
